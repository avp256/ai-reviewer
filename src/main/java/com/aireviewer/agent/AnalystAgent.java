...
public class AnalystAgent implements Agent {
    ...
    public void analyse(MergeRequestContext mrContext, JiraContext jiraContext, AIReviewComment comment) {
        ...
        if (jiraContext != null && jiraContext.getSummary() != null) {
            comment.setDoneWell("Зміни в цілому відповідають описаним вимогам: " + jiraContext.getSummary());
        } else {
            comment.setDoneWell("Немає достатньої інформації про контекст задачі, проте структура виглядає узгоджено.");
        }
    }
}